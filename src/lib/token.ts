...
let copilotBootstrapPromise: Promise<void> | null = null
let copilotModelsPromise: Promise<void> | null = null

export const copilotBootstrapDependencies = {
  cacheModels,
  getGitHubUser,
  getCopilotUsage,
  getCopilotToken,
...
async function ensureStoredGitHubToken(): Promise<void> {
  if (state.githubToken) {
    return
  }

  const githubToken = await copilotBootstrapDependencies.readGitHubToken()
  if (!githubToken) {
    throw new HTTPError(
...
async function ensureCopilotModelsLoaded(): Promise<void> {
  if (state.models) {
    return
  }

  copilotModelsPromise ??= copilotBootstrapDependencies
    .cacheModels()
    .finally(() => {
      copilotModelsPromise = null
    })

  await copilotModelsPromise
}

export async function ensureCopilotBootstrapped(options?: {
  loadModels?: boolean
}): Promise<void> {
  copilotBootstrapPromise ??= (async () => {
    const previousGitHubToken = state.githubToken
    const previousUserName = state.userName
    const previousCopilotApiUrl = state.copilotApiUrl
    const previousCopilotToken = state.copilotToken

    try {
      await ensureStoredGitHubToken()

      if (!state.userName || !state.copilotApiUrl) {
        await logUser()
      }
      ...
      state.githubToken = previousGitHubToken
      state.userName = previousUserName
      state.copilotApiUrl = previousCopilotApiUrl
      state.copilotToken = previousCopilotToken
      throw error
    }
  })().finally(() => {
    copilotBootstrapPromise = null
  })

  await copilotBootstrapPromise
...
export const setupCopilotToken = async () => {
  ...
  const { token, refresh_in } =
    await copilotBootstrapDependencies.getCopilotToken()
...
export async function logUser() {
  const user = await copilotBootstrapDependencies.getGitHubUser()
  state.userName = user.login
  consola.info(`Logged in as ${user.login}`)

  const copilotUser = await copilotBootstrapDependencies.getCopilotUsage()
