...
test("Bun fetch uses the proxy env for HTTP requests when initialized", async () => {
  ...
  Object.defineProperty(fetchMock, "preconnect", {
    configurable: true,
    value: preconnect,
  })
  ...
  try {
    ...
    await fetch("https://api.githubcopilot.com/models")

    expect(dispatcher).toBeDefined()
    expect((fetch as typeof fetch & { preconnect?: unknown }).preconnect).toBe(
      preconnect,
    )
    expect(fetchMock).toHaveBeenCalledTimes(1)
    expect(capturedInit?.proxy).toBe("http://127.0.0.1:8080")
  } finally {
